...
def list_available_pages() -> List[Dict[str, Any]]:
    """Lists metadata for documentation pages accessible to the authenticated session.
...
def fetch_and_sanitize_page(page_id: str) -> str:
    """Retrieves the sanitized Markdown content of a documentation page, enforcing RBAC.
...
def semantic_search_accelerator(query: str, top_k: int = 3) -> List[Dict[str, Any]]:
    """Performs an RBAC-secure semantic similarity search over the documentation index.
