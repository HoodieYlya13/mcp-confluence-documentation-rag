...
Alongside the URL, the page `title` is threaded onto `DocumentChunk` (read from `doc.metadata["title"]`, where it already lived for `list_available_pages`) and emitted on the search serialization, so the chunk carries a human-readable label, not just the numeric `doc_id`. The live `doc_id` is an opaque Confluence page id (`983042`); the title is what the citation links should read as. The field is optional throughout — the mock corpus has no title key, so offline chunks carry an empty title and every consumer falls back to `doc_id`, keeping offline output byte-identical.
...
**Grounded source citations.** A successful generation gets a `**Sources:**` footer listing the unique pages actually retrieved, as Markdown links labelled by page title (falling back to `doc_id` when no title is present). To keep this faithful rather than a fabricated claim, both the page URL and its title are injected into each context-block header (`[Source: … | Title: … | URL: … | Score: …]`) — so the LLM-as-judge sees the title and URL in the context and the citation is grounded, not hallucinated. The footer is appended only on the success path: greetings, refusals, leak-blocks, the unavailable fallback, and the exact "no relevant documents" response (extracted to `NO_CONTEXT_RESPONSE` and reused by the prompt) are all skipped, so the refusal/no-context strings the tests and eval match on stay byte-exact. With the air-gapped local source the URLs are empty, so no footer is produced and offline output is unchanged.
...
- **HTTP, OIDC tokens:** the same bearer can also be a real OIDC access token (`resolve_role_from_sso_token`). `SSO_ISSUER`/`SSO_AUDIENCE`/`SSO_JWKS_URL` each accept one value or a bracketed list, so multiple identity providers are trusted at once (the issuer is read from the token and matched against the allowlist; its keys come from the positionally-paired JWKS URL, or `{issuer}/.well-known/jwks.json` by default — Keycloak needs an explicit `{issuer}/protocol/openid-connect/certs`). Verification is RS256 over the JWKS, issuer-in-allowlist, `exp`/`iat`, and an **audience-or-`azp`** match (Keycloak puts the client in `azp` while `aud` lists unrelated resource servers). The `roles` claim maps to a clearance (`ADMIN`/`ADMIN_DURNAL`/`ATS_CORE_LEAD` → lead, `JUNIOR_OP` → junior); any failure fails closed.
- **Demo-only signature bypass (`SSO_INSECURE_ISSUER`):** the hosted demo Space cannot reach the corporate Keycloak behind the company VPN/firewall, so the JWKS fetch the signature check needs is unreachable. Issuers listed in `SSO_INSECURE_ISSUER` therefore skip JWKS/signature verification *only* — issuer-allowlist, `aud`/`azp`, and `exp`/`iat` are still enforced, and each bypass logs a `signature_bypassed` event. It is safe here because the demo exposes only the bundled fictional mocked content (no real protected files), is secure-by-default (empty unless set), and **must stay empty in production**; the production issuer `auth.hy13dev.com` is publicly reachable and always fully verified. Full rationale in `SECURITY.md`.
